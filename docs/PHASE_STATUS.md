...
| 4 | Build | Ready for review | `claude/ui-4-stack-viz-wb437s` | `npm run check` passes (lint, 120 tests, build). 18 centred week rows, 71 blocks, documented span map, completed/planned/missed states, single newest-block glow, compact legend, and a read-only workout detail sheet. Verified at 320px, 390px, and 1280px and with reduced motion via a fixed-clock Playwright pass; no horizontal overflow at any width, and keyboard focus and Enter/Space activation confirmed. |
| 5 | Plan | Not started |  |  |
