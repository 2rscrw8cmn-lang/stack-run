...
- **D-075 correction:** Member Build mileage is no longer the member-summary `Miles Built` number. It is `CrewMiniBuildModel.totalMiles`, the sum of the exact blocks the card/sheet draws, so the printed number can never disagree with the tower. The Crew comparison `Miles Built` metric still reads the Crew-windowed member-summary value and is unchanged. Complete `blockPlacements`, placement timestamps/internal state and the rest of AppState stay private.
...
- `crews.build_start_date` is the authoritative contribution boundary for the shared communal Crew Build, Recent Crew Runs and crew-relative comparison stats. `completedDate >= buildStartDate` is eligible for those views; membership join time, import time, plan linkage and local creation time are irrelevant. It is not a boundary on what a member may upload — see D-071.
- New Crew creation defaults Build starts to today and owner editing validates it on or before race day. Moving later uses an explicit confirmation and one owner-only server transaction that demotes (never deletes) pre-window `shared_runs` rows off the Crew Build for all members, cascades their Props and recursively demotes unsupported construction to READY without relocation. Moving earlier deletes or invents nothing; normal additive projection contributes each member's newly eligible local history.
- **D-071 correction:** projection uploads a member's full local run history regardless of `buildStartDate` — Member Build is a sanitized reproduction of the runner's real Personal Build, not a crew-scoped artifact, so it must not be date-clipped. `buildStartDate` is still embedded in the projection fingerprint (crew-relative summary stats stay windowed) but no longer filters what gets projected/uploaded. RLS on `shared_runs` INSERT/UPDATE no longer rejects pre-window dates; the window is enforced only in `place_crew_build_block`, which rejects placing a pre-window run onto the shared communal tower. Dashboard reads keep `miniBuildRuns` (Member Build) unwindowed while `runs` (Recent Crew Runs) and `crewBuildRuns` stay windowed. `crew_members.joined_at` remains history/order/audit only.
