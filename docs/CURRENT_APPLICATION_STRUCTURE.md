...
- UI-4 Build screen — earned blocks and placement (D-014):
  ...
  - `src/features/build/` holds the screen: `BuildScreen`, `BuildMetrics` (unchanged), `PendingBlocksTray`, `BuiltStructure` → `BuiltCourseRow` → `PlacedBlock`, `PlaceBlockSheet` → `PlacementGrid`, and `BuildLegend`.
  - Placement is tap-to-place. `PlacementGrid` renders the week's band of courses, each a five-column CSS grid with every child in the same row: background cells, the blocks already built, a preview of the block being placed, and one button per valid start column. Only valid positions are buttons, so the tab order walks exactly the real choices; each is named `Place Intervals block in Week 6, course 2, columns 3 through 4`. Focus or hover previews the block's full width, because the button is one column wide while the block may be four.
  ...
  - The Place Block grid deliberately stays front on. It is the precision interaction, and an angled five-column grid is harder to aim at on a phone.
  - The full plan builds a 36-course tower about 190px wide, so the structure grows upward as the weeks pass instead of sitting as an 18-row slab.
