...
This correction is not UI-23. `crew_members.joined_at`, converted to the runner's browser-local calendar date, is the authoritative participation boundary. Projection upserts only runs whose `completedDate` is on or after that date; same-day runs count, late imports after joining count, and pre-membership runs remain entirely personal. `cleanup_pre_membership_shared_runs` intentionally removes older server projections and Props, then demotes any unsupported surviving Crew blocks to READY without relocating them. Ordinary device absence remains non-destructive.
