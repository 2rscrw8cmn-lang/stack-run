...
- Owns the global Settings sheet and its Account & Crew and Run Data child sheets. Runs owns its local `YOU | CREW` context, focused Training Signal and personal/crew-safe run-detail sheets. Crew empty states can open the existing Account & Crew settings without creating another destination.
...
`src/features/crew/CrewRunsView.tsx` renders crew/race/member identity and the approved Weekly Miles, Long Run, Consistency and Miles Built facts. Equal values retain membership order rather than inventing tie-breakers. Consistency shows percentage plus completed/due, while zero due is the neutral `—`. The current account receives a quiet `YOU` marker and lime edge rather than a podium treatment. One-member, signed-out, no-crew, unavailable and no-shared-run states are intentional.

`CrewRunRow.tsx` and `CrewRunDetailSheet.tsx` consume only `CrewSharedRun`. Pace is derived from shared distance/duration. Even the current runner's row opens the same crew-safe detail, with no edit/delete/private-data actions. UI-20 reactions, comments, member profiles and mini Builds remain deferred.
