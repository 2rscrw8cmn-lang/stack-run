...
export async function serveCrewInviteImage(request: Request): Promise<Response> {
...
export default async function handler(first: unknown, second?: unknown): Promise<Response | void> {
  if (!isNodeResponse(second)) return serveCrewInviteImage(first as Request);
  ...
  const response = await serveCrewInviteImage(new Request(`https://image.invalid${request.url ?? "/api/og/crew-invite"}`, {
