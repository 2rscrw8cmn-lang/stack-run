...
describe("Crew Build placement RPC client", () => {
  it("sends only the shared run id and snapped Crew coordinates", async () => {
    ...
    await placeCrewBuildBlock(client, { sharedRunId: "run-1", row: 3, columnStart: 4, rotated: false });
  ...
  it("classifies the server's specific collision without auto-relocating", async () => {
    ...
    await expect(
      placeCrewBuildBlock(client, { sharedRunId: "run-1", row: 0, columnStart: 1, rotated: false }),
  ...
  it.each([
    ...
    await expect(
      placeCrewBuildBlock(client, { sharedRunId: "run-1", row: 2, columnStart: 1, rotated: false }),
