...
import { PlanActualColumns, type PlanActualColumn } from "./PlanActualColumns";
...
describe("PlanActualColumns", () => {
  it("makes the chart itself the week selector, with no separate visible pill row", () => {
    ...
    expect(screen.getByRole("button", { name: /^Week 1,/ })).toHaveAttribute("aria-pressed", "false");
  });

  it("selects a week when its chart region is activated", async () => {
